...
/******/ 	__webpack_require__.p = "/static/lib/";
/******/
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _httpUtil = __webpack_require__(7);
	...
	_vue2.default.use(_constants2.default);
	_vue2.default.use(_httpUtil2.default);
	_validate2.default.init(_vue2.default);
	...
	router.beforeEach(_filters2.default.loginFilter());
...
/***/ function(module, exports, __webpack_require__) {
	...
	var spellUrl = function spellUrl(_ref) {
	  var url = _ref.url;
	  var _ref$query = _ref.query;
	  var query = _ref$query === undefined ? {} : _ref$query;
	
	  if (url.lastIndexOf('?') < 0) url += '?1=1';
	  if (url.endsWith('&')) url = url.slice(0, -1);
	
	  for (var k in query) {
	    if (query[k] === null || query[k] === undefined) continue;
	    if (query[k] instanceof Array) {
	      for (var v in query[k]) {
	        url += '&' + k + '=' + v;
	      }
	    } else {
	      url += '&' + k + '=' + query[k];
	...
	var install = function install(Vue) {
	  ...
	  var headers = {
	    'x-user-id': _commonUtil2.default.getUserId(),
	    'Content-Type': 'application/json'
	  };
	
	  var getReq = function getReq(url, query) {
	    console.log(url);
	    console.log(query);
	    headers.timestamp = Date.now();
	    Vue.http.get(spellUrl(url, query), headers).then(function (data) {
	      return _promise2.default.resolve(JSON.parse(data.data));
	    }, function (err) {
	      return _promise2.default.reject(JSON.parse(err.data));
	    });
	  };
	
	  var postReq = function postReq(url, data) {
	    console.log(url);
	    console.log(data);
	    headers.timestamp = Date.now();
	    return Vue.http.post(url, data, headers).then(function (data) {
	      return _promise2.default.resolve(JSON.parse(data.data));
	    }, function (err) {
	      return _promise2.default.reject(JSON.parse(err.data));
	    });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _blueimpMd = __webpack_require__(22);
	...
	exports.encryptPassword = function (phoneNum, password) {
	  return (0, _blueimpMd2.default)(phoneNum + password);
	};
	...
	module.exports = {
	  ...
	  methods: {
	    submit: function submit() {
	      var passWd = _commonUtil2.default.encryptPassword(this.info.phoneNum, this.info.password),
	          url = _vue2.default.constants.AUTH,
	          self = this;
	      _vue2.default.postReq(url, { phoneNum: self.info.phoneNum, password: passWd }).then(function (data) {
	        _commonUtil2.default.loginInit(data);
	        self.$router.go('/secretary');
	      }, function (err) {
	        alert(err.msg);
	      });
	    }
